...
func TestMain(t *testing.T) {
	testCommand(t, `echo 'Hello there!' | xo '/hello(.*)/Hi$1/i'`,
		`Hi there!
`)
	testCommand(t, `echo 'Hello! - Bob' | xo '/(hello).*?-.*?(\w+)/Why $1, $2!/i'`,
		`Why Hello, Bob!
`)
	testCommand(t, `cat fixtures/servers.yml | xo '/.*?(production):\s*server:\s+([^:\n]+):?(\d+)?.*?user:\s+([^\n]+).*/$4@$2 -p $3?:22/mis'`,
		`user-1@192.168.1.1 -p 1234
`)
	testCommand(t, `cat fixtures/starwars.txt | xo '/^(\w+):(\s*\[(.*?)\]\s*)?\s*([^\n]+)/$1 said, "$4" in a $3?:normal voice./mi'`,
...
`)
	testCommand(t, `echo '123' | xo '/(\d)(\d)(\d)(\d)?(\d)?/$1, $2, $3, 4?:FOUR $5?:FIVE/'`,
		`1, 2, 3, 4?:FOUR FIVE
`)
